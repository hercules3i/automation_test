...
def __selectUnit(browser, task):
    try:
        ...
        logger.warning(f"{task.card_job} - Khong co don vi hoac khong co du lieu: {task.unit}")
...
def __selectDuration(browser, task):
    try:
        ...
        logger.warning(f"{task.card_job} - Khong co thoi han hoac khong co du lieu: {task.duration}")
...
def __select_staff(browser,task):
    try:
        ...
        logger.warning(f"{task.card_job} - Khong co nhan vien ten: {task.staff}")
...
def __findToAddStaff(browser,task):
    ...
    try:
            ...
            logger.error(f"{task.card_job} - Element not found: {e}")
    except Exception as e:
            logger.error(f"{task.card_job} - An error occurred: {e}", exc_info=True)

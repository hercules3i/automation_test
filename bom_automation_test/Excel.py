...
def LoadSimpleData() -> list:
    ...
    for row in range(2, max_row):
        ...
        if in_or_out == "Nhập" or in_or_out == "Nhập ":
            warehouse_obj = new_bom.warehouse_in
        elif in_or_out == "Xuất" or in_or_out == "Xuất ":
            warehouse_obj = new_bom.warehouse_out

        #detect warehouse's input and output
        in_out_value = sheet.cell(row, 6).value
        if in_out_value.__str__().strip() == "Đầu vào" or in_out_value.__str__().strip() == "Đầu vào ":
            warehouse_prod = warehouse_obj.input
        elif in_out_value.__str__().strip() == "Đầu ra" or in_out_value.__str__().strip() == "Đầu ra ":
        ...
        product.recoverable_cost=sheet.cell(row,13).value
        product.scrap=sheet.cell(row,14).value
        product.lost_rati=sheet.cell(row,15).value
        product.cost=sheet.cell(row,16).value
        product.steel_type=sheet.cell(row,17).value
        if not product.IsNone():
            warehouse_prod.product_list.append(product)

    return list_tasks

def LoadSimpleData2() -> list:
    sheet = work_book["Sheet1"]
    max_column = sheet.max_column
    max_row = sheet.max_row
    ...
    for row in range(2, max_row):
        ...
        if working in list_workings2:
            ...
            latest_in_or_out = None
        #collect bom-details
        work_order = sheet.cell(row, 2).value #"none"
        if work_order is None and latest_work_order != '': work_order = latest_work_order

        shift = sheet.cell(row, 3).value #ca2 13:30 - 17:30 
        if shift is None: shift = latest_shift

        machines = sheet.cell(row, 4).value #BF24-01
        if machines is None: machines = latest_machines

        workers = sheet.cell(row, 5).value  # Phamba vuong
        if workers is None: workers = latest_workers

        in_or_out = sheet.cell(row, 6).value #nhap
        if in_or_out is None: in_or_out = latest_in_or_out
        ...
        #detect warehouse's input and output
        in_out_value = sheet.cell(row, 7).value
        ...
        if warehouse_obj is not None:
            product = warehouse_obj.Product()
        product.name = sheet.cell(row, 8).value
        if product.name is not None:
            product.name = product.name.rstrip("\n\t")
        product.quantity = sheet.cell(row, 9).value
        product.unit = sheet.cell(row, 10).value
        product.specification = sheet.cell(row, 11).value
        product.tail = sheet.cell(row, 12).value
        product.not_good = sheet.cell(row, 13).value
        product.recoverable_cost = sheet.cell(row,13).value
        product.scrap = sheet.cell(row, 13).value
        product.lost_rati = sheet.cell(row, 14).value
        product.cost = sheet.cell(row, 15).value
        product.steel_type = sheet.cell(row, 16).value
        if not product.IsNone():
            warehouse_prod.product_list.append(product)
